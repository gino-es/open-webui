<script lang="ts">
	...
	const helpText = {
		...
		verbosity: $i18n.t('Set logging verbosity level for debugging and monitoring'),
		tools: $i18n.t('JSON array of tools to enable. Example: [{"type": "function", "name": "get_weather"}, {"type": "mcp", "server_label": "deepwiki"}]')
	};
	...
	// Validate JSON format for tools
	$: toolsValidation = (() => {
		if (!responsesConfig.tools || responsesConfig.tools.trim() === '') {
			return { valid: true, error: null };
		}
		try {
			const parsed = JSON.parse(responsesConfig.tools);
		...
		{#if responsesConfig.enabled}
			<div class="ml-6 space-y-3 border-l-2 border-gray-200 dark:border-gray-700 pl-4">
				...
				<div class="flex items-center gap-3">
					...
					<select
						bind:value={responsesConfig.verbosity}
						class="text-xs bg-transparent border border-gray-300 dark:border-gray-600 rounded px-2 py-1"
					>
						<option value="low">{$i18n.t('Low')}</option>
						<option value="medium">{$i18n.t('Medium')}</option>
					...
					<Tooltip content={marked.parse(helpText.verbosity)}>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							fill="none"
							viewBox="0 0 24 24"
				...
				<div class="space-y-2">
					<div class="flex items-center gap-3">
						...
						<Tooltip content={marked.parse(helpText.tools)}>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="none"
								viewBox="0 0 24 24"
					...
					<textarea
						bind:value={responsesConfig.tools}
						placeholder={`[{"type": "function", "name": "get_weather"}, {"type": "mcp", "server_label": "deepwiki"}]`}
						class="w-full text-xs bg-transparent border border-gray-300 dark:border-gray-600 rounded px-2 py-1 font-mono resize-y"
						rows="4"
						class:border-red-500={!toolsValidation.valid}
						class:border-green-500={toolsValidation.valid && responsesConfig.tools && responsesConfig.tools.trim() !== ''}
					/>
					{#if !toolsValidation.valid}
						<div class="text-xs text-red-500">
							{toolsValidation.error}
						</div>
					{/if}
					{#if toolsValidation.valid && responsesConfig.tools && responsesConfig.tools.trim() !== ''}
